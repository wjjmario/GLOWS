...
def rotation_consistent_feature(encoder, image, cache_path=None):
    if cache_path and os.path.exists(cache_path):
        try:
            return F.normalize(
                torch.load(
                    cache_path, map_location=encoder.device, weights_only=True
                ).float(),
                dim=1,
            )
        ...
        if path and os.path.exists(path):
            try:
                results[index] = F.normalize(
                    torch.load(
                        path, map_location=encoder.device, weights_only=True
                    ).float(),
                    dim=1,
                )
            except (OSError, RuntimeError, EOFError, ValueError):
        ...
        for k in (0, 1, 2, 3):
            oriented = [
                (
                    images[index]
                    if k == 0
                    else Image.fromarray(np.ascontiguousarray(np.rot90(array, k=k)))
                )
...
def point_patch_embedding(feature, x, y, image_size, side):
    ...
    fx = int(round(float(x) / max(image_size[0] - 1, 1) * (w - 1)))
    fy = int(round(float(y) / max(image_size[1] - 1, 1) * (h - 1)))
    x0 = max(0, min(fx - (side - 1) // 2, w - side))
    y0 = max(0, min(fy - (side - 1) // 2, h - side))
    return F.normalize(
        feature[0, :, y0 : y0 + side, x0 : x0 + side].mean((-2, -1)), dim=0
    )


def adaptive_anchor_prototypes(feature, nodes, instance_map, points, image_size, cfg):
    hcfg = cfg.get("hybrid_anchor", {})
    default_side = int(hcfg.get("default_patch_tokens", 2))
    patch_sides = {
        int(k): int(v) for k, v in hcfg.get("patch_tokens_by_class", {}).items()
    }
    car_id = int(hcfg.get("car_class_id", 4))
    car_side = int(hcfg.get("car_patch_tokens", 1))

    def patch_side(cid):
        return patch_sides.get(
            int(cid), car_side if int(cid) == car_id else default_side
        )

    sam_weight = float(hcfg.get("sam_weight", 0.7))
    patch_weight = 1.0 - sam_weight
    by_id = {int(n["mask_id"]): n for n in nodes}
    ...
    if max_points <= 1 and bool(hcfg.get("legacy_matching", True)):
        ...
        for cid, class_points in points.items():
            ...
            for x, y in class_points:
                ...
                xi = int(np.clip(round(x), 0, inst.shape[1] - 1))
                yi = int(np.clip(round(y), 0, inst.shape[0] - 1))
                mid = int(inst[yi, xi])
                if mid > 0 and mid in by_id:
                    vec = F.normalize(
                        sam_weight * by_id[mid]["embedding"] + patch_weight * patch,
                        dim=0,
                    )
    ...
    if max_points > 1:
        ...
        for cid, patch, mid in matched:
            unambiguous = (
                mid > 0 and mid in by_id and classes_by_mask.get(mid, set()) == {cid}
            )
            if unambiguous and (cid, mid) not in used_by_class:
                vec = F.normalize(
                    sam_weight * by_id[mid]["embedding"] + patch_weight * patch, dim=0
                )
    ...
    for cid, values in values_by_class.items():
        if values:
            vectors[cid] = F.normalize(torch.stack(values).mean(0), dim=0)
    return vectors, direct_anchors, patch_only


class _UF:
    def __init__(self, ids):
        self.p = {int(x): int(x) for x in ids}
...
def complete_graph_forest(nodes, threshold=0.8, max_area=0.25, image_area=1):
    by = {int(n["mask_id"]): n for n in nodes}
    ids = sorted(by)
    uf = _UF(ids)
    comp = {
        i: {
            "area": float(by[i]["area"]),
            "sum": by[i]["embedding"] * float(by[i]["area"]),
        }
        for i in ids
    }
    trees = {i: i for i in ids}
    edges = []
    for aidx, a in enumerate(ids):
        for b in ids[aidx + 1 :]:
            edges.append(
                (float(torch.dot(by[a]["embedding"], by[b]["embedding"])), a, b)
            )
    for edge, a, b in sorted(edges, reverse=True):
        ...
        if ca["area"] + cb["area"] > float(image_area) * float(max_area):
            continue
        cohesion = float(
            torch.dot(F.normalize(ca["sum"], dim=0), F.normalize(cb["sum"], dim=0))
        )
        if min(edge, cohesion) < float(threshold):
            continue
        ta, tb = trees[ra], trees[rb]
        uf.union(ra, rb)
        root = uf.find(ra)
        comp[root] = {"area": ca["area"] + cb["area"], "sum": ca["sum"] + cb["sum"]}
        trees[root] = (ta, tb)
...
def refine_forest_with_points(forest, direct_anchors):
    ...
    def cut(tree):
        leaves = _leaves(tree)
        classes = {direct_anchors[m] for m in leaves if m in direct_anchors}
        if len(classes) <= 1 or isinstance(tree, int):
            groups.append((leaves, classes))
            return
        cut(tree[0])
        cut(tree[1])
...
def _group_embedding(leaves, by):
    ...
    for mid in leaves:
        if mid in by:
            vec.append(by[mid]["embedding"])
            weights.append(float(by[mid]["area"]) ** 0.5)
    if not vec:
        return None
    x = torch.stack(vec)
...
def build_hybrid_pseudo(
    ...
    nodes = proposal_nodes(feature, instance_map)
    by = {int(n["mask_id"]): n for n in nodes}
    local_prototypes, anchors, patch_only = adaptive_anchor_prototypes(
        feature, nodes, instance_map, points, image.size, cfg
    )
    ...
    if bool(gcfg.get("enabled", True)):
        forest = complete_graph_forest(
            nodes,
            gcfg.get("threshold", 0.8),
            gcfg.get("max_area_ratio", 0.25),
            np.asarray(instance_map).size,
        )
        ...
        groups = [
            (
                [int(node["mask_id"])],
                (
                    {anchors[int(node["mask_id"])]}
                    if int(node["mask_id"]) in anchors
                    else set()
                ),
            )
            for node in nodes
    ...
    records = []
    assigned = set()
    accept = float(gcfg.get("class_threshold", 0.55))
    margin = float(gcfg.get("class_margin", 0.05))
    for gid, (leaves, classes) in enumerate(groups, 1):
        ...
        if len(classes) == 1:
            ...
            for cid, proto in prototypes.items():
                ...
                if decoder_scores:
                    decoder_values = [
                        decoder_scores.get(m, {}).get(cid, 0.0) for m in leaves
                    ]
                    decoder_value = float(
                        np.nan_to_num(
                            np.mean(decoder_values), nan=0.0, posinf=0.0, neginf=0.0
                        )
                    )
            ...
            if scores[0][0] < accept or scores[0][0] - second < margin:
                continue
            conf = float(np.clip(scores[0][0], 0, 1))
            cid = int(scores[0][1])
            source = "dino_group"
        for mid in leaves:
            if mid in assigned:
    ...
    excluded = set(
        int(x)
        for x in cfg.get("point_supervision", {}).get("exclude_propagated_classes", [])
    )
    if excluded:
        records = [
            r
            for r in records
            if not (int(r.class_id) in excluded and r.source == "dino_group")
        ]
    available = [c for c in class_ids if int(c) in prototypes]
    if available:
        p = torch.stack([prototypes[int(c)] for c in available])
        logits = torch.einsum("bchw,kc->bkhw", feature, p)
        ...
        idx = logits.argmax(1)[0].cpu().numpy()
        semantic = np.zeros((image.height, image.width), dtype=np.uint16)
        for i, cid in enumerate(available):
            semantic[idx == i] = int(cid)
        confidence = logits.softmax(1).max(1).values[0].cpu().numpy()
    else:
        semantic = np.full((image.height, image.width), 255, dtype=np.uint16)
        confidence = np.zeros_like(semantic, dtype=np.float32)
    point_fallbacks = []
    return {
        "feature": feature,
        "nodes": nodes,
        ...
        "anchors": anchors,
        "patch_only_classes": patch_only,
        "groups": groups,
        "point_fallbacks": point_fallbacks,
    }
