...
class PositionEmbeddingSine(nn.Module):
    ...
    def forward(self, x):
        ...
        dim = self.temperature ** (
            2 * torch.div(dim, 2, rounding_mode="floor") / self.num_pos_feats
        )
...
class PixelDecoder(nn.Module):
    def __init__(
        self, in_dim, hidden_dim=256, mask_dim=256, memory_mode="same_resolution"
    ):
        super().__init__()
        self.memory_mode = str(memory_mode).lower()
        if self.memory_mode != "same_resolution":
            raise ValueError("GLOWS Full uses same_resolution memories")
        self.projections = nn.ModuleList(
            [nn.Conv2d(in_dim, hidden_dim, 1) for _ in range(4)]
        )
    ...
    def forward(self, tokens, patch_h, patch_w):
        ...
        return self.mask_features(fused), features
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, query, memory, query_pos, memory_pos, attention_mask=None):
        ...
        cross, _ = self.cross(
            q, memory + memory_pos, memory, attn_mask=attention_mask, need_weights=False
        )
        query = query + cross
...
class ObjectFormer(nn.Module):
    ...
    def __init__(
        ...
        self.pixel_decoder = PixelDecoder(
            in_dim, hidden_dim, hidden_dim, self.memory_mode
        )
        ...
        self.level_embedding = nn.Embedding(4, hidden_dim)
        ...
        if use_prototype_head:
            raise ValueError("GLOWS Full does not use an auxiliary prototype head")

    def _prediction(self, query, mask_features):
        normalized = self.decoder_norm(query)
        logits = self.class_head(normalized)
        masks = torch.einsum(
            "bqc,bchw->bqhw", self.mask_head(normalized), mask_features
        )
        prediction = {
            "pred_logits": logits,
            "pred_masks": masks,
    ...
    def _attention_mask(self, masks, memory_hw):
        resized = F.interpolate(
            masks, size=memory_hw, mode="bilinear", align_corners=False
        )
        ...
        return (
            blocked[:, None].expand(-1, self.num_heads, -1, -1).flatten(0, 1).detach()
        )
    ...
    def forward(self, tokens, patch_h, patch_w):
        ...
        for i, layer in enumerate(self.layers):
            ...
            pos = (
                (
                    self.position(fmap)
                    + self.level_embedding.weight[level][None, :, None, None]
                )
                .flatten(2)
                .transpose(1, 2)
            )
            attention_mask = self._attention_mask(
                outputs[-1]["pred_masks"], fmap.shape[-2:]
            )
        ...
        final["aux_outputs"] = [
            {
                key: x[key]
                for key in ("pred_logits", "pred_masks", "pred_prototypes")
                if key in x
            }
            for x in outputs[:-1]
        ]
...
def build_model(cfg, num_classes, encoder_dim):
    ...
    return ObjectFormer(
        ...
        use_prototype_head=bool(mcfg.get("use_prototype_head", False)),
        memory_mode=str(mcfg.get("memory_mode", "same_resolution")),
    )
