...
from objectformer.objects import instances_to_targets
from objectformer.prompts import build_point_prompts, canonicalize_point_prompts
from objectformer.prototypes import EpochPrototypeAccumulator
from objectformer.proposals import SAM2ProposalGenerator
from objectformer.self_training import (
    EMAModel,
    point_signature,
    proposal_decoder_scores,
    reconcile_instances,
)
...
def build_criterion(cfg, num_classes):
    ...
    return SetCriterion(
        ...
        matcher,
        {
            "loss_ce": lcfg.get("class_weight", 2.0),
            "loss_mask": lcfg.get("mask_weight", 5.0),
            "loss_dice": lcfg.get("dice_weight", 5.0),
            "loss_proto": lcfg.get("prototype_weight", 1.0),
        },
...
def should_update(epoch, cfg):
    ...
    start, interval = int(ucfg.get("start_epoch", 3)), max(
        1, int(ucfg.get("interval", 1))
    )
...
def semantic_batch_loss(
    outputs, pseudos, class_to_index, device, confidence_weighted=False
):
    ...
    loss_map = F.cross_entropy(
        prediction.float(), target, ignore_index=255, reduction="none"
    )
    ...
    return torch.nan_to_num(
        (numerator / denominator).mean(), nan=0.0, posinf=10.0, neginf=0.0
    )
    ...
    torch.save(
        {
            ...
            "global_prototypes": {
                int(k): v.cpu() for k, v in global_prototypes.items()
            },
            "anchor_prototypes": {
                int(k): v.cpu() for k, v in anchor_prototypes.items()
            },
            "config": cfg,
...
def main():
    ...
    parser.add_argument("--points", type=int, choices=[1, 3], default=None)
    parser.add_argument("--batch-size", type=int, default=None)
    args = parser.parse_args()

    cfg = load_yaml(args.config)
    if args.points is not None:
        cfg["prompt"]["points_per_class"] = args.points
    ...
    items = (
        train_set.items if args.max_images < 0 else train_set.items[: args.max_images]
    )
    tcfg = cfg.get("train", {})
    out_dir = ensure_dir(
        args.output_dir or tcfg.get("output_dir", "outputs/objectformer")
    )
    cfg["train"]["output_dir"] = out_dir
    if not items:
        raise ValueError("No training images found")
    if any(item.get("label") is None for item in items):
        raise ValueError(
            "Training requires a mask for point simulation for every image"
        )
    ckpt_dir = ensure_dir(os.path.join(out_dir, "checkpoints"))
    bank = PseudoInstanceBank(os.path.join(out_dir, "pseudo_bank"))
    save_json(
        {
            "config": args.config,
            "device": args.device,
    ...
    sam2 = SAM2ProposalGenerator(
        cfg["proposal"], device, dataset_name=os.path.basename(cfg["dataset"]["root"])
    )
    model = build_model(cfg, len(class_ids), encoder.embed_dim).to(device)
    ema = EMAModel(
        model, decay=float(cfg.get("pseudo_update", {}).get("ema_decay", 0.999))
    )
    ...
    if args.resume:
        ...
        global_prototypes = {
            int(k): v.to(device) for k, v in state.get("global_prototypes", {}).items()
        }
        anchor_prototypes = {
            int(k): v.to(device) for k, v in state.get("anchor_prototypes", {}).items()
        }
    ...
    prototype_fields = [
        "prototype_classes",
        "prototype_candidates_seen",
        "prototype_candidates_accepted",
        "prototype_rejected_confidence",
        "prototype_rejected_dino",
        "prototype_rejected_ema",
        "prototype_anchor_count",
        "prototype_candidate_count",
        "prototype_drift_mean",
    ]
    for cid in class_ids:
        prototype_fields.extend(
            [f"anchor_count_{cid}", f"candidate_count_{cid}", f"prototype_drift_{cid}"]
        )
    closed_loop_fields = (
        ["epoch", "global_enabled", "persistent_bank", "agreement_gate"]
        + bank_fields
        + prototype_fields
    )
    ...
    with open(log_path, "a", newline="", encoding="utf-8") as log_file, open(
        ...
        closed_loop_writer = csv.DictWriter(
            closed_loop_file, fieldnames=closed_loop_fields
        )
        if write_header:
            writer.writeheader()
        ...
        for epoch in range(start_epoch, epochs + 1):
            ...
            epoch_global_prototypes = (
                {
                    cid: vector.detach().clone()
                    for cid, vector in global_prototypes.items()
                }
                if global_enabled and epoch >= global_use_epoch
                else {}
            )
            ...
            for batch_index, (batch_start, batch_items) in enumerate(progress):
                ...
                for local_index, (item, image, label) in enumerate(
                    zip(batch_items, images, labels)
                ):
                    image_index = batch_start + local_index
                    points = build_point_prompts(
                        label, cfg, class_ids, seed_offset=image_index
                    )
                    previous_payload = bank.load(train_set.split, item["stem"])
                    previous_records = []
                    instance_map = sam2.generate(
                        image, stem=item["stem"], split=train_set.split
                    )
                    points = canonicalize_point_prompts(
                        points,
                        instance_map,
                        points_per_class=int(
                            cfg.get("prompt", {}).get("points_per_class", 1)
                        ),
                    ...
                    if (
                        ...
                        and previous_payload.get("point_signature") == signature
                    ):
                        previous_records = previous_payload.get("instances", [])
                    points_batch.append(points)
                    maps.append(instance_map)
                    previous_batch.append(previous_records)
                    signatures.append(signature)

                tokens = [
                    x.to(device).float()
                    for x in encoder.extract_intermediate_tokens_batch(images)
                ]
                ...
                rotation_cache_paths = [
                    (
                        os.path.join(
                            rotation_cache_root, train_set.split, item["stem"] + ".pt"
                        )
                        if rotation_cache_root
                        else None
                    )
                    for item in batch_items
                ]
                features = rotation_consistent_features(
                    encoder, images, rotation_cache_paths
                )
                ...
                for local_index, (
                    ...
                    image_index = batch_start + local_index
                    ema_outputs = (
                        slice_outputs(ema_batch_outputs, local_index)
                        if ema_batch_outputs is not None
                        else None
                    )
                    decoder_scores = (
                        proposal_decoder_scores(ema_outputs, instance_map, class_ids)
                    ...
                    pseudo = build_hybrid_pseudo(
                        ...
                        global_prototypes=epoch_global_prototypes,
                        feature=feature,
                    )
                    nodes, proposed_records, anchors = (
                        pseudo["nodes"],
                        pseudo["records"],
                        pseudo["anchors"],
                    )
                    persistent_bank = bool(
                        cfg.get("pseudo_update", {}).get("persistent_bank", False)
                    )
                    if persistent_bank:
                    ...
                    if global_enabled:
                        ...
                        if epoch >= global_update_epoch:
                            prototype_accumulator.add_candidates(
                                nodes,
                                records,
                                pseudo["prototypes"],
                                decoder_scores,
                    ...
                    supervised_records = records
                    target_records = sorted(
                        supervised_records,
                        key=lambda record: (
                            int(record.anchored),
                            float(np.nan_to_num(record.confidence, nan=0.0)),
                        ),
                        reverse=True,
                    )[:max_targets]
                    targets.append(
                    ...
                    supervised_record_counts.append(len(supervised_records))
                ...
                with torch.autocast(
                    ...
                    semantic_weight = float(
                        cfg.get("loss", {}).get("semantic_weight", 1.0)
                    )
                    if semantic_weight:
                        losses["loss_semantic"] = semantic_batch_loss(
                            ...
                            confidence_weighted=bool(
                                cfg.get("loss", {}).get(
                                    "semantic_confidence_weighted", False
                                )
                            ),
                        )
                    else:
                    ...
                    losses["loss_total"] = (
                        losses["loss_total"] + semantic_weight * losses["loss_semantic"]
                    )
                ...
                if scaler_enabled:
                    ...
                    loss.backward()
                step = (batch_index + 1) % accumulation == 0 or batch_index + 1 == len(
                    batches
                )
                if step:
                    if scaler_enabled:
                    ...
                    try:
                        grad_norm = torch.nn.utils.clip_grad_norm_(
                            model.parameters(),
                            float(tcfg.get("clip_grad_norm", 0.1)),
                ...
                row = {
                    ...
                    "loss_ce": float(losses["loss_ce"].detach()),
                    "loss_mask": float(losses["loss_mask"].detach()),
                    "loss_dice": float(losses["loss_dice"].detach()),
                    "loss_proto": float(losses["loss_proto"].detach()),
                    "loss_semantic": float(losses["loss_semantic"].detach()),
            ...
            log_file.flush()
            if global_enabled:
                global_prototypes, anchor_prototypes, prototype_stats = (
                    prototype_accumulator.finalize(
                        global_prototypes,
                        anchor_prototypes,
                        cfg,
                    )
            ...
            closed_loop_row = {
                ...
                "persistent_bank": int(
                    bool(cfg.get("pseudo_update", {}).get("persistent_bank", False))
                ),
                "agreement_gate": int(
                    bool(cfg.get("agreement_gate", {}).get("enabled", False))
                ),
                **bank_epoch_stats,
                **prototype_stats,
            }
            ...
            save_json(
                closed_loop_row,
                os.path.join(out_dir, "closed_loop", f"epoch_{epoch:04d}.json"),
            )
            checkpoint(
            ...
            if (
                int(tcfg.get("save_every", 0)) > 0
                and epoch % int(tcfg["save_every"]) == 0
            ):
                checkpoint(
            ...
            if (
                int(tcfg.get("eval_every", 1)) > 0
                and epoch % int(tcfg.get("eval_every", 1)) == 0
            ):
                ...
                print(
                    {
                        "epoch": epoch,
                        "image_only_test": report,
                        "closed_loop": closed_loop_row,
                        **mean,
                    }
                )
